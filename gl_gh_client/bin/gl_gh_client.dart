          ...
          request.map(
            start: (request) async {
              ...
              process.stdout
                  ...
                  .map(WsResponseMessage.stdout)
                  .listen(addWsResponse, onDone: stdoutCompleter.complete);
              process.stderr
                  .map((event) {
                    stderr.add(event);
                    return event;
                  })
                  .map(WsResponseMessage.stderr)
                  .listen(addWsResponse, onDone: stderrCompleter.complete);

              await Future.wait([stdoutCompleter.future, stderrCompleter.future]);

              stdout.writeln(await process.exitCode);
